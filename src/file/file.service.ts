...
import { S3Service } from 'src/services/aws/services/s3.service';
...
export class FileService {
  ...
  async createFile(createFileDto: CreateFileDto, user: User) {
    ...
    try {
      ...
      const key = `${user.id}/${createFileDto.folder}/${savedFile.id}-${timeStamp}.${mime.extension(savedFile.mimeType)}`;
      const metadata = {
        userid: user.id,
        label: createFileDto.label,
        fileName: createFileDto.file.originalname,
